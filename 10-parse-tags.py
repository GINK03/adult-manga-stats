...
def _map(arg):
  ...
  for name in names:
    try:
      ...
      fav = soup.find('span', {'class':'m-txtDetailProductInfoFavoriteNum'}).text.strip().replace(',', '')
      fav = int(fav)
...
args = {}
for index, name in enumerate(Path('../scraping-designs/dmm-adult-book-scrape/htmls').glob('*')):
  key = index%16
